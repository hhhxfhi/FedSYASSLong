...
class ClientBase:
    def __init__(self, args, client_id, trainset, testset, statistic):
        ...
        self.flType = args.flType  # 联邦学习类型，例如HtFL异构联邦学习
        ...
        if self.flType == "HtFL":
            self.model = BaseHeadSplit(args, self.client_id).to(self.device)    # 将模型划分成特征提取器和+分类头
    ...
    def extractFeature(self, bestEpoch):
        best_model_path = "/root/autodl-tmp/FedPSY226/results/kronoDroid/" + str(
            self.args.algorithm) + "-models/" + str(bestEpoch)
