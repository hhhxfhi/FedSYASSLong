...
def prepare_modelsDIM(args):
    if args.dataset == "fmnist":
        args.num_classes = 10  # 类别数
        args.in_channels = 1  # 输入通道数
        args.FedAvgCNNDim = 1024  # 全连接层输入维度
        args.feature_dim = 84  # 中间特征维度
...
def prepare_models(args):
    ...
    if args.modelName == 'LeNet5':
        ...
        args.model = FedAvgCNN(in_features=args.in_channels, num_classes=args.num_classes, dim=args.FedAvgCNNDim).to(
            args.device)
...
def run(args):
    ...
    for repeat_time in range(args.num_repeat_times):
        ...
        if args.flType == "HtFL":
            prepare_modelsDIM(args)  # 设置args的num_classes、in_channels、FedAvgCNNDim、feature_dim
            # Generate args.models
            """ 根据异构特征提取器HtFEX中的X，设置整个系统中使用的不同模型架构，模型架构差异 """
            ...
            # HtC4表示4种个性化分类头，每种head表示一个不同的多层感知机（MLP），用于个性化分类器。Head结构差异
            ...
            # 定义了8种不同的CNN，用于模拟客户端模型架构的差异性。卷积层+隐层数量差异
            ...
            if args.algorithm == "LG":
                ...
                args.head = nn.Linear(args.feature_dim, args.num_classes)   # 分类头
                args.projection = nn.Linear(args.feature_dim, args.feature_dim)     # 投影头，特征提取之后，分类之前
        ...
        result = {"statistic": args.statistic, "acc_record": acc_record, "GlobalClassAcc": server.global_class_acc,
                  "ClientAcc": server.client_acc}
...
if __name__ == '__main__':
    ...
    if args.debug:
        print("debug")
    if args.benignAlone:  # 指定是否每个客户端都有一个纯良性数据子集
        print("benign alone")
    # 用于调试和记录实验设定
